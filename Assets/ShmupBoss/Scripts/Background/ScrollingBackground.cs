...
namespace ShmupBoss
{
    ...
    public class ScrollingBackground : PoolBase
    {
        ...
        public override GameObject Spawn(string goName)
        {
            ...
            if (GoQueueByName[goName].Count <= 0)
            {
                return null;
            }
            ...
            if (backgroundLayer.TreadmillTransform.gameObject.activeInHierarchy)
            {
                pulledGO.transform.SetParent(backgroundLayer.TreadmillTransform);
            }
            ...
            for (int i = 0; i < backgroundLayers.Length; i++)
            {
                ...
                for (int j = 0; j < backgroundLayers[i].Backgrounds.Length; j++)
                {
                    for (int k = 0; k < backgroundLayers[i].Backgrounds[j].Count; k++)
                    {
                        ...
                        previewBG.transform.SetParent(previewBackgrounds.transform);
